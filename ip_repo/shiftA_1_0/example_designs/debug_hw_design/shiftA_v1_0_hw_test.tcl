# Runtime Tcl commands to interact with - shiftA_v1_0

# Sourcing design address info tcl
set bd_path [get_property DIRECTORY [current_project]]/[current_project].srcs/[current_fileset]/bd
source ${bd_path}/shiftA_v1_0_include.tcl
...
# Test: CLK
# Create a write transaction at clk_addr address
create_hw_axi_txn w_clk_addr [get_hw_axis $jtag_axi_master] -type write -address $clk_addr -data $wdata_1
# Create a read transaction at clk_addr address
create_hw_axi_txn r_clk_addr [get_hw_axis $jtag_axi_master] -type read -address $clk_addr
# Initiate transactions
run_hw_axi r_clk_addr
run_hw_axi w_clk_addr
run_hw_axi r_clk_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_clk_addr]]
# Compare read data
if { $rdata_tmp == $wdata_1 } {
	puts "Data comparison test pass for - CLK"
} else {
	puts "Data comparison test fail for - CLK, expected-$wdata_1 actual-$rdata_tmp"
	inc ec
}
...
# Test: Q
set wdata_q [get_init_data 0]
create_hw_axi_txn w_q_addr [get_hw_axis $jtag_axi_master] -type write -address $axi_gpio_out_addr -data ${wdata_q}
create_hw_axi_txn r_q_addr [get_hw_axis $jtag_axi_master] -type read -address $axi_gpio_in_addr 
# Initiate transactions
run_hw_axi r_q_addr
run_hw_axi w_q_addr
run_hw_axi r_q_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_q_addr]]
set DE [ get_done_and_error_bit $rdata_tmp 1 0 ]
# Compare read data
if { $DE == 01 } {
	puts "Data comparison test pass for - Q"
} else {
	puts "Data comparison test fail for - Q, rdata-$rdata_tmp expected-01 actual-$DE"
	inc ec
}
